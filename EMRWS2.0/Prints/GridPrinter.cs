...
class GridPrinter
{
    ...
    public GridPrinter(DataGridView objDataGridView, PrintDocument objPrintDocument, bool bCenterOnPage, bool bHasTitle, string sTitle, string header, Font objHeaderFont, Color objHeaderColor, Font objTitleFont, Color objTitleColor, bool bPaging)
    {
        ...
        paging = bPaging;
        this.header = header;
        this.headerFont = objHeaderFont;
        this.headerColor = objHeaderColor;
        pageNumber = 0;
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        {
            pageNumber++;
            string PageString = "病历质量检查评分标准" + pageNumber.ToString();
        ...
        if (hasTitle)
        {
           ...
           RectangleF HeaderRectangle = new RectangleF(currentX, currentY, (float)pageWidth - (float)rightMargin - (float)leftMargin, g.MeasureString(header, headerFont).Height);

         
            g.DrawString(title, titleFont, new SolidBrush(titleColor), TitleRectangle, TitleFormat);
            g.DrawString(header, headerFont, new SolidBrush(headerColor), HeaderRectangle, HeaderFormat);

            currentY += g.MeasureString(header, titleFont).Height;
        }
        float CurrentX = (float)leftMargin;
        ...
        Color HeaderForeColor = Color.Black;// dataGridView.ColumnHeadersDefaultCellStyle.ForeColor;
